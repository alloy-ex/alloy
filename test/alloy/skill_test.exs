defmodule Alloy.SkillTest do
  use ExUnit.Case, async: true

  alias Alloy.Skill

  setup do
    base = Path.join(System.tmp_dir!(), "alloy_skill_#{System.unique_integer([:positive])}")
    File.mkdir_p!(base)

    home = Path.join(base, "home")
    project = Path.join(base, "project")

    File.mkdir_p!(Path.join(home, ".alloy/skills"))
    File.mkdir_p!(Path.join(project, ".alloy/skills"))
  ...
  describe "discover/1" do
    test "discovers skills from global directory", %{home: home} do
      File.write!(Path.join(home, ".alloy/skills/review.md"), """
      ---
      name: review
    ...
    test "discovers skills from project directory", %{home: home, project: project} do
      File.write!(Path.join(project, ".alloy/skills/deploy.md"), """
      ---
      name: deploy
    ...
    test "project skills override global skills with same filename", %{
      ...
      File.write!(Path.join(project, ".alloy/skills/review.md"), """
      ---
      name: review
    ...
    test "merges global and project skills with different names", %{
      ...
      File.write!(Path.join(home, ".alloy/skills/global.md"), """
      ---
      name: global-skill
      ---
      Global body
      ...
      File.write!(Path.join(project, ".alloy/skills/project.md"), """
      ---
      name: project-skill
      ---
      Project body
    ...
    test "ignores non-.md files", %{home: home} do
      File.write!(Path.join(home, ".alloy/skills/valid.md"), "Skill body")
      File.write!(Path.join(home, ".alloy/skills/invalid.txt"), "Not a skill")
