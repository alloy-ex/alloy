defmodule Alloy.Context.DiscoveryTest do
  ...
  setup do
    ...
    File.mkdir_p!(Path.join(home, ".alloy/context"))
    File.mkdir_p!(Path.join(project, ".git"))
    File.mkdir_p!(Path.join(project, ".alloy/context"))
    File.mkdir_p!(cwd)
  ...
  describe "discover/1" do
    ...
    test "discovers global context files from ~/.alloy/context/", %{home: home, cwd: cwd} do
      File.write!(Path.join(home, ".alloy/context/rules.md"), "Be concise")
    ...
    test "discovers project context files from git root", %{
      ...
      File.write!(Path.join(project, ".alloy/context/project.md"), "Project context")
    ...
    test "merges global and project files in order (global first)", %{
      ...
      File.write!(Path.join(home, ".alloy/context/alpha.md"), "Global alpha")
      File.write!(Path.join(project, ".alloy/context/beta.md"), "Project beta")
    ...
    test "sorts files alphabetically within each tier", %{home: home, cwd: cwd} do
      File.write!(Path.join(home, ".alloy/context/zebra.md"), "Z content")
      File.write!(Path.join(home, ".alloy/context/alpha.md"), "A content")
    ...
    test "ignores non-.md files", %{home: home, cwd: cwd} do
      File.write!(Path.join(home, ".alloy/context/notes.md"), "Keep me")
      File.write!(Path.join(home, ".alloy/context/notes.txt"), "Ignore me")
      File.write!(Path.join(home, ".alloy/context/config.yaml"), "Ignore me too")
    ...
    test "discovers cwd-level context files (third tier)", %{
      ...
      File.mkdir_p!(Path.join(cwd, ".alloy/context"))
      File.write!(Path.join(cwd, ".alloy/context/local.md"), "CWD context")
      File.write!(Path.join(project, ".alloy/context/project.md"), "Project context")
    ...
    test "handles missing .alloy/context directories gracefully" do
      nowhere =
        Path.join(System.tmp_dir!(), "alloy_nowhere_#{System.unique_integer([:positive])}")
    ...
    test "detects git root by walking parent dirs", %{
      ...
      File.write!(Path.join(project, ".alloy/context/found.md"), "Found via git root")
    ...
    test "does not duplicate when cwd IS the git root", %{home: home, project: project} do
      File.write!(Path.join(project, ".alloy/context/only_once.md"), "Once")
  ...
  describe "Config integration" do
    test "context_discovery: true merges discovered files into system_prompt", %{
      ...
      File.write!(Path.join(home, ".alloy/context/rules.md"), "Always be concise.")
      File.write!(Path.join(project, ".alloy/context/project.md"), "This is the Alloy project.")
      ...
      assert config.system_prompt =~ "This is the Alloy project."
    end
    ...
    test "context_discovery: true with nil system_prompt uses discovered content as prompt", %{
      ...
      File.write!(Path.join(home, ".alloy/context/rules.md"), "Discovered context only.")
