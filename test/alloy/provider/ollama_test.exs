defmodule Alloy.Provider.OllamaTest do
  ...
  describe "stream/4" do
    test "emits text chunks and returns correct response" do
      ...
      assert {:ok, result} = Ollama.stream([Message.user("Hi")], [], config, on_chunk)
      assert result.stop_reason == :end_turn
      assert [%Message{role: :assistant}] = result.messages
    ...
    test "accumulates tool calls without emitting chunks" do
      ...
      chunks =
        collect_chunks(fn on_chunk ->
          Ollama.stream([Message.user("Read mix.exs")], [], config, on_chunk)
        end)
    ...
    test "request body includes stream: true" do
      ...
      Ollama.stream([Message.user("Hi")], [], config, fn _ -> :ok end)
  ...
  defp config_with_sse_stream(chunks) do
    %{model: "llama3.2", req_options: [plug: {Req.Test, __MODULE__}, retry: false]}
