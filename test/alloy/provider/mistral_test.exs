defmodule Alloy.Provider.MistralTest do
  ...
  describe "stream/4" do
    test "emits text chunks and returns correct response" do
      ...
      assert {:ok, result} = Mistral.stream([Message.user("Hi")], [], config, on_chunk)
      assert result.stop_reason == :end_turn
      assert [%Message{role: :assistant}] = result.messages
    ...
    test "accumulates tool calls without emitting chunks" do
      ...
      chunks =
        collect_chunks(fn on_chunk ->
          Mistral.stream([Message.user("Read mix.exs")], [], config, on_chunk)
        end)
    ...
    test "request body includes stream: true" do
      ...
      Mistral.stream([Message.user("Hi")], [], config, fn _ -> :ok end)
  ...
  defp config_with_sse_stream(chunks) do
    %{api_key: "mistral-test-key", model: "mistral-large-latest", req_options: [plug: {Req.Test, __MODULE__}, retry: false]}
