defmodule Anvil.Middleware.LoggerTest do
  use ExUnit.Case

  import ExUnit.CaptureLog

  alias Anvil.Middleware.Logger, as: AnvilLogger
  ...
  describe "before_completion hook" do
    test "logs debug message with turn number" do
      ...
      log =
        capture_log(fn ->
          result = AnvilLogger.call(:before_completion, state)
          assert result == state
        end)
  ...
  describe "after_completion hook" do
    test "logs info message with status" do
      ...
      log =
        capture_log(fn ->
          result = AnvilLogger.call(:after_completion, state)
          assert result == state
        end)
  ...
  describe "after_tool_execution hook" do
    test "logs debug message about tool execution" do
      ...
      log =
        capture_log(fn ->
          result = AnvilLogger.call(:after_tool_execution, state)
          assert result == state
        end)
  ...
  describe "on_error hook" do
    test "logs error message with error details" do
      ...
      log =
        capture_log(fn ->
          result = AnvilLogger.call(:on_error, state)
          assert result == state
        end)
