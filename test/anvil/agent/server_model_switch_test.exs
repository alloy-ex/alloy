defmodule Anvil.Agent.ServerModelSwitchTest do
  ...
  describe "set_model/2" do
    test "changes provider module and config" do
      ...
      assert :ok =
               Server.set_model(agent,
                 provider: {Anvil.Provider.Test, agent_pid: pid2}
               )
    ...
    test "preserves conversation history after switch" do
      ...
      :ok = Server.set_model(agent, provider: {Anvil.Provider.Test, agent_pid: pid2})
    ...
    test "preserves other config fields (tools, system_prompt, max_turns, etc.)" do
      ...
      assert state.config.provider == Anvil.Provider.Test
    ...
    test "accepts a bare module atom (no config keyword list)" do
      ...
      :ok = Server.set_model(agent, provider: Anvil.Provider.Test)
