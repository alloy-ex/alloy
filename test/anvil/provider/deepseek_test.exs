defmodule Anvil.Provider.DeepSeekTest do
  use ExUnit.Case, async: true

  alias Anvil.Provider.DeepSeek
  alias Anvil.Message
  ...
  describe "complete/3 with text response" do
    test "returns normalized end_turn response" do
      ...
      assert {:ok, result} = DeepSeek.complete(messages, [], config)
      assert result.stop_reason == :end_turn
      assert [%Message{role: :assistant}] = result.messages
      assert Message.text(hd(result.messages)) == "Hello from DeepSeek!"
  ...
  describe "complete/3 message formatting" do
    test "sends authorization header" do
      ...
      DeepSeek.complete([Message.user("Hi")], [], config)

      assert_received {:request_headers, headers}
      auth = Enum.find(headers, fn {k, _v} -> k == "authorization" end)
      assert {"authorization", "Bearer ds-test-key"} = auth
    end

    test "uses correct DeepSeek API URL" do
      config = config_that_captures_request()
    ...
    test "includes system prompt as system message" do
      config =
        ...
        |> Map.put(:system_prompt, "You are DeepSeek.")
      ...
      assert system_msg["content"] == "You are DeepSeek."
    end
    ...
    test "includes tool definitions in request" do
      ...
      DeepSeek.complete([Message.user("Hi")], tool_defs, config)
  ...
  describe "complete/3 error handling" do
    test "returns error on HTTP failure" do
      ...
      assert {:error, _reason} = DeepSeek.complete([Message.user("Hi")], [], config)
    end
    ...
    test "returns error on API error response" do
      ...
      assert {:error, reason} = DeepSeek.complete([Message.user("Hi")], [], config)
