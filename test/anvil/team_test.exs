defmodule Anvil.TeamTest do
  ...
  defp agent_opts(responses, overrides \\ []) do
    ...
    Keyword.merge([provider: {Anvil.Provider.Test, agent_pid: pid}], overrides)
  end
  ...
  describe "shared context" do
    ...
    test "initial shared_context can be provided at start" do
      {:ok, team} =
        Team.start_link(
          agents: [],
          shared_context: %{project: "anvil", version: "0.2"}
        )

      assert Team.get_context(team, :project) == "anvil"
