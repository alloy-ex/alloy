defmodule Anvil.Scheduler do
  @moduledoc """
  GenServer for time-based agent runs (cron jobs and heartbeats).

  Schedules `Anvil.run/2` calls at regular intervals using
  ...
  ## Usage
      ...
      {:ok, pid} = Anvil.Scheduler.start_link(
        jobs: [
          %{
            name: :ci_check,
            ...
            agent_opts: [
              provider: {Anvil.Provider.Anthropic, api_key: "sk-ant-...", model: "claude-haiku-4-5-20251001"},
  ...
  - `:prompt` — the prompt string passed to `Anvil.run/2`
  - `:agent_opts` — options passed to `Anvil.run/2`
  ...
  def handle_info({:tick, name}, state) do
    state =
      if MapSet.member?(state.running, name) do
        Logger.warning("Anvil.Scheduler: skipping #{name} (still running)")
  ...
  def handle_info({:DOWN, ref, :process, _pid, reason}, state) do
    case Map.pop(state.task_refs, ref) do
      ...
      {name, task_refs} ->
        Logger.error("Anvil.Scheduler: job #{name} crashed: #{inspect(reason)}")
        state = %{state | running: MapSet.delete(state.running, name), task_refs: task_refs}
        {:noreply, state}
    end
  end
      ...
      job ->
        task =
          Task.Supervisor.async_nolink(state.task_supervisor, fn ->
            Anvil.run(job.prompt, job.agent_opts)
          end)
  ...
  defp default_on_result({:ok, result}) do
    Logger.info("Anvil.Scheduler: job completed — #{result.text || "(no text)"}")
  end

  defp default_on_result({:error, result}) do
    Logger.error("Anvil.Scheduler: job failed — #{inspect(result.error)}")
  end
end
