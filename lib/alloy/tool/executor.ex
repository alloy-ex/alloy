defmodule Alloy.Tool.Executor do
      ...
      {:ok, tagged} ->
        {results, meta} =
          ...
          |> Task.async_stream(
            ...
            timeout: tool_timeout, ordered: true, on_timeout: :kill_task
          )
          |> Enum.zip(tagged)
          |> Enum.map(fn
            {{:ok, pair}, _} -> pair
            {{:exit, reason}, tag} -> crashed(call_from(tag), reason, on_event, seq_ref, corr_id, turn)
          end)
          |> Enum.unzip()
  ...
  defp run_tagged({:execute, call}, fns, ctx, on_event, seq_ref, corr_id, turn) do
        ...
        {:ok, mod} ->
          try do
            case mod.execute(call[:input] || %{}, ctx) do
              {:ok, r} -> {Message.tool_result_block(call[:id], r), nil}
              {:error, r} -> {Message.tool_result_block(call[:id], r, true), r}
            end
          rescue
            e ->
              err = "Tool crashed: #{Exception.message(e)}"
              {Message.tool_result_block(call[:id], err, true), err}
          end
    ...
    meta = %{id: call[:id], name: call[:name], input: call[:input] || %{}, duration_ms: ms, error: error}
    eseq = emit_end(on_event, meta, seq_ref, corr_id, turn, sseq)
    {result, Map.merge(meta, %{correlation_id: corr_id, start_event_seq: sseq, end_event_seq: eseq})}
  end

  defp run_tagged({:blocked, call, reason}, _, _, on_event, seq_ref, corr_id, turn) do
    sseq = emit_start(on_event, call, seq_ref, corr_id, turn)
    error = "Blocked: #{reason}"
    meta = %{id: call[:id], name: call[:name], input: call[:input] || %{}, duration_ms: 0, error: error}
    eseq = emit_end(on_event, meta, seq_ref, corr_id, turn, sseq)
  ...
  defp emit_start(on_event, call, seq_ref, corr_id, turn) do
    ...
    on_event.({:tool_start, %{id: call[:id], name: call[:name], input: call[:input] || %{}, event_seq: seq, correlation_id: corr_id}})
    :telemetry.execute([:alloy, :tool, :start], %{event_seq: seq}, %{correlation_id: corr_id, turn: turn, tool_id: call[:id], tool_name: call[:name]})
    seq
  end

  defp emit_end(on_event, meta, seq_ref, corr_id, turn, start_seq) do
    seq = :atomics.add_get(seq_ref, 1, 1)
    on_event.({:tool_end, Map.merge(meta, %{event_seq: seq, correlation_id: corr_id, start_event_seq: start_seq})})
    :telemetry.execute([:alloy, :tool, :stop], %{event_seq: seq, duration_ms: meta.duration_ms || 0}, %{correlation_id: corr_id, turn: turn, tool_id: meta.id, tool_name: meta.name, error: meta.error, start_event_seq: start_seq})
    seq
  end
  ...
  defp build_context(%State{} = state) do
    Map.merge(state.config.context, %{working_directory: state.config.working_directory, config: state.config})
  end
end
