defmodule Mix.Tasks.Anvil do
  use Mix.Task

  @shortdoc "Run the Anvil agent interactively (or one-shot with -p)"

  @moduledoc """
  Run the Anvil agent harness from the terminal.
  ...
  ## Interactive mode (default)
      ...
      mix anvil
      mix anvil --provider gemini --tools read,bash
      mix anvil "Start with this question"   # sends first message, then prompts

  ## Programmatic mode (-p)

      mix anvil -p "What is the capital of France?"
      mix anvil -p "Read mix.exs and tell me the version" --tools read
      mix anvil -p "Write a haiku" --provider openai --model gpt-4o
  ...
  @providers %{
    "anthropic" => {Anvil.Provider.Anthropic, "ANTHROPIC_API_KEY", "claude-opus-4-6"},
    "gemini" => {Anvil.Provider.Google, "GEMINI_API_KEY", "gemini-2.0-flash"},
    "google" => {Anvil.Provider.Google, "GEMINI_API_KEY", "gemini-2.0-flash"},
    "openai" => {Anvil.Provider.OpenAI, "OPENAI_API_KEY", "gpt-4o"}
  }
  ...
  defp run_programmatic("", _agent_opts, _provider, _model, _tools) do
    Mix.shell().error("""
    Usage: mix anvil -p "your message" [--provider anthropic|gemini|openai] [--tools read,bash]

    Run `mix help anvil` for full documentation.
    """)
  ...
  defp run_programmatic(message, agent_opts, provider_name, model, tools) do
    Mix.shell().info("[anvil] #{provider_name}/#{model}" <> tool_label(tools))
    Mix.shell().info("")

    case Anvil.run(message, agent_opts) do
      {:ok, result} ->
  ...
  defp run_interactive(opening_message, agent_opts, provider_name, model, tools) do
    Mix.shell().info("[anvil] #{provider_name}/#{model}" <> tool_label(tools) <> " · interactive")
    ...
    {:ok, pid} = Anvil.Agent.Server.start_link(agent_opts)
  ...
  defp handle_skill_command(pid, message, skills) do
    ...
    case Enum.find(skills, &(&1.name == skill_name)) do
      ...
      skill ->
        expanded = Anvil.Skill.apply(skill, input)
  ...
  defp handle_usage(pid) do
    usage = Anvil.Agent.Server.usage(pid)
    ...
    Token usage:
      ...
      Total:         #{Anvil.Usage.total(usage)}
    """)
  end

  defp handle_history(pid) do
    messages = Anvil.Agent.Server.messages(pid)
  ...
  defp format_history_line(%Anvil.Message{role: role, content: content})
  ...
  defp format_history_line(%Anvil.Message{role: role, content: blocks})
  ...
  defp handle_reset(pid) do
    :ok = Anvil.Agent.Server.reset(pid)
      ...
      case Map.get(@providers, provider_name) do
        ...
        {mod, env_var, default_model} ->
          ...
          if api_key do
            ...
            :ok =
              Anvil.Agent.Server.set_model(pid,
  ...
  defp chat_and_print(pid, message) do
    ...
    if streaming? do
      ...
      case Anvil.Agent.Server.stream_chat(pid, message, on_chunk) do
        {:ok, result} ->
          IO.write("\n")
      ...
      case Anvil.Agent.Server.chat(pid, message) do
        {:ok, result} ->
          Mix.shell().info("\nAnvil: #{result.text || "(no text response)"}")
          tokens = result.usage.input_tokens + result.usage.output_tokens
